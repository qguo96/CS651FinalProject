...
path = "../training_data/price/Nov_22_28.csv"
...
df2 = df.withColumn("Date", df["Date"].substr(1, 13))


df3 = df2.withColumn("Date", F.regexp_replace('Date', ' ', '-'))
#df3.show()

w = Window().orderBy(desc("Timestamp"))
df4 = df3.withColumn("Futureprice", F.lag("Close").over(w))
#df4.show()
...
df5 = df4_fill.withColumn("id", row_number().over(w))
#df5.show()
...
results.write.option("header", "true").mode("overwrite").csv("../training_data/price/processed_Nov_22_28")